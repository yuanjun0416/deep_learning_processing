...
def main():
    ...
    ## ImageNet所用图片的RGB三通道的均值
    _R_MEAN = 123.68
    _G_MEAN = 116.78
    _B_MEAN = 103.94

    '''
    使用迁移学习时，使用别人的训练模型参数，就必须和别人使用相同的预处理方法
    '''
    def pre_function(img):
        # img = im.open('test.jpg')
        # img = np.array(img).astype(np.float32)
        img = img - [_R_MEAN, _G_MEAN, _B_MEAN]  ## 本文的迁移学习模型预训练参数就是使用了这种数据预处理方法
    ...
    ## 当我们将trainable设置为False时，所有权重都会被冻结，训练过程中也无法训练这些参数, training设置为True也不行
